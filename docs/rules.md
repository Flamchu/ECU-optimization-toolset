...
rationale so it surfaces in `--help` output and in the generated Markdown
report. Thresholds live in
...
| R16 | EGR observed | `max egr_duty > 2 %` anywhere in log | critical | global | Software EGR delete is mandatory. Any EGR duty means the delete was not flashed, was applied to only one bank, or was overridden by spec-MAF polarity. |
...
| R20 | Cruise spec-MAF excess | `MAF_actual − MAF_spec ≥ 50 mg` with EGR = 0 | info | per-pull | Strategy-B confirmation: spec-MAF map fill is conservative — soft alert that the delete is functional. |
| R21 | Idle stability | RPM σ > 25 over 30-s warm-idle window (`coolant_c ≥ 70`, `pedal_pct < 5`, `vehicle_speed = 0`) | warn (info if window < 30 s OR σ > 15) | global | Catches injector / mech imbalance; gates the conditional `Idle_fuel` recommendation. |
| R22 | Low-pedal IQ slope | Linear regression of `iq_requested` on `pedal_pct` in 5..25 %; fires Warn if `slope_low > 0.40 mg/pct` OR `slope_low / slope_mid > 1.8` | warn | per-pull | Off-idle "lunge" guard. SKIPPED if `pedal_pct` missing, low-band has fewer than 30 samples, all rpm < 1000, or coolant cold. |
| R23 | Sustained-pull coolant trend | `dT ≥ 10 °C` arms the rule; `T_peak ≥ 102 °C` fires Warn, otherwise Info | info / warn | per-pull | Validates that the cooling-fan threshold and run-on tune is doing useful work. SKIPPED if coolant missing, pull < 5 s, or rpm never ≥ 2500. |
...
For each row in the 25-row default-deltas table the recommendation engine
emits exactly one of:

- `APPLY` — at least one triggering rule fired (or the row is
  unconditional) AND the resulting value stays inside the envelope.
